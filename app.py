...
def callback():
    ...
    if 'email' in user_info and user_info['email'] == 'mr.shashansoni@gmail.com':
...
def add_menu_item():
    ...
    image_filename = secure_filename(image.filename)  # Get a secure file name
...
def menu():
    ...
    if request.method == "POST":
        ...
        if item_name:
            cart = session["cart"]
            item = next(item for item in MENU_ITEMS if item["name"] == item_name)
